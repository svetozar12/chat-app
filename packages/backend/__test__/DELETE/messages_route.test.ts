...
beforeAll(async () => {
  try {
    ...
    await request(app).post(`/messages/${test_chat_room._id}`).send({ sender: dumyUser.username, message: "testingMsg", seenBy: [] });
    return true;
  } catch (error) {
    return false;
  }
...
describe(`Testing endpoint :/users/${dumyUser.username}`, () => {
  it("should return 200 OK", async () => {
    const res = await request(app).delete(`/messages/${chat_id}`).send({ sender: dumyUser.username, message: "testingMsg" });
    expect(res.status).toBe(200);
    expect(res.body.message).toBe(`Message testingMsg has been deleted`);
  });
  it("should return 404 Not found user", async () => {
    const res = await request(app).delete(`/messages/${chat_id}`).send({ sender: "nonExistent", message: "testingMsg" });
    expect(res.status).toBe(404);
    expect(res.body.message).toBe(`User: nonExistent not found`);
  });
  it("should return 404 Not found message", async () => {
    const res = await request(app).delete(`/messages/${chat_id}`).send({ sender: dumyUser.username, message: "nonExistentMessage" });
    expect(res.status).toBe(404);
