...
beforeAll(async () => {
  const res = await request(app)
    ...
    .send({ user_id: user1_id, inviter: dumyUser.username, reciever: dumyUser2.username });
  const updateInvite = await request(app)
    .put("/invites")
    .set({ Authorization: `Bearer ${tokens.Access_token}` })
    .send({ user_id: user1_id, id: res.body.message._id, status: "accepted" });
...
describe("Creating chat-room :/chat-room", () => {
  ...
  it("should return 404 User Not found", async () => {
    const res = await request(app)
      .post("/chat-room")
      .set({ Authorization: `Bearer ${tokens.Access_token}` })
      .send({ user_id: user1_id, invite_id, user1: "nonExistent1", user2: "nonExistent2" });
...
describe("Creating group chat :/invites/group-chat", () => {
  it("should return 201 Created", async () => {
    const res = await request(app)
      ...
      .set({ Authorization: `Bearer ${tokens.Access_token}` })
      .send({ user_id: user1_id, usersData: [dumyUser.username, dumyUser2.username, dumyUser4.username] });
  ...
  it("should return 404 Not found", async () => {
    const res = await request(app)
      .post("/invites/group-chat")
      .set({ Authorization: `Bearer ${tokens.Access_token}` })
      .send({ user_id: user1_id, usersData: ["nonExistent1", "nonExistent2"] });
    expect(res.text).toBe('{"ErrorMsg":"User nonExistent1 not found"}');
    expect(res.status).toBe(404);
  });
});
